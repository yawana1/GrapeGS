    ...
    if(plot){
      ...
      if(method == "VanRaden"){
        for(i in 1:length(Ga)){
          # pdf = pdf(paste("heatmap_", filename, names(Ga)[i],".pdf", sep = ""), width = 10, height = 4)
          png(file = paste("heatmap_", filename, names(Ga)[i],".png", sep = ""),
              width = 1040,
              height = 600
          )
        ...
        pdf = pdf(paste("heatmap_", filename, ".pdf", sep = ""), width = 10, height = 4)
